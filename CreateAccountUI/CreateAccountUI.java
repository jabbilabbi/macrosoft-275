...
public class CreateAccountUI implements ActionListener{
	...
	public void addComponentsToPane(Container pane) {
		...
		passwordConflict = new JLabel("Your confirmed password is different, please re-type");
		passwordConflict.setVisible(false);
		
		usernameTaken = new JLabel("Your username is in use, please use another");
		usernameTaken.setVisible(false);
		
		somethingNotEntered = new JLabel("Some fields have been left empty, please complete the form");
		...
		passwordConflict.setBounds(200 + insets.left, 450 + insets.top,
				size.width, size.height);
		
		size = usernameTaken.getPreferredSize();
		usernameTaken.setBounds(200 + insets.left, 450 + insets.top,
				size.width, size.height);
		
		size = somethingNotEntered.getPreferredSize();
		somethingNotEntered.setBounds(200 + insets.left, 450 + insets.top,
				size.width, size.height);
	}
	// PRE: None
	...
	public void actionPerformed(ActionEvent e) {
		...
		if ((usernameTextBox.getText().length() == 0) || (passwordTextBox.getPassword() == null) || (passwordConfirmTextBox.getPassword() == null) || (secretQuestionTextBox.getText() == "") || (secretAnswerTextBox.getText() == "")) 
			{
			somethingNotEntered.setVisible(true);
			if (passwordTextBox.getPassword() == passwordConfirmTextBox.getPassword())
				{
				passwordConflict.setVisible(false);
				if (!sdb.getUserNames().contains(usernameTextBox.getText()))
					{
					usernameTaken.setVisible(false);
					sdb.appendLoginDatabase("logins.txt", usernameTextBox.getText(), passwordTextBox.getText(), secretQuestionTextBox.getText(), secretAnswerTextBox.getText());
					}
				else{
